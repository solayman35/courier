...
class courier{
    ...
    public function employee_login($email,$password){
      echo $query="SELECT * FROM employee WHERE email='$email' AND password='$password' ";
    ...
    public function getReport($to_upazila,$date) {
     $query="SELECT * FROM report WHERE from_upazila='$to_upazila' OR to_upazila='$to_upazila' or date='$date'";
    $result= mysqli_query($this->db, $query);
        return $result;
